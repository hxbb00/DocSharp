    ...
    private bool ProcessBreakControlWord(RtfControlWord cw, FormattingState runState)
    {
        ...
        switch(name)
        {
            case "line":
                ...
                {
                    EnsureRun().Append(new Break() { Type = BreakValues.TextWrapping });
            ...
            case "column":
                ...
                EnsureRun().Append(new Break() { Type = name == "page" ? BreakValues.Page : BreakValues.Column });
            ...
            case "lbr":
                ...
                if (cw.HasValue && !runState.LastWasLineBreak)
                {
                    if (cw.Value!.Value == 0)
                    {
                        EnsureRun().Append(new Break() { Type = BreakValues.TextWrapping, Clear = BreakTextRestartLocationValues.None });
                        runState.LastWasLineBreak = true;
                    }
                    else if (cw.Value.Value == 1)
                    {
                        EnsureRun().Append(new Break() { Type = BreakValues.TextWrapping, Clear = BreakTextRestartLocationValues.Left });
                        runState.LastWasLineBreak = true;
                    }
                    else if (cw.Value.Value == 2)
                    {
                        EnsureRun().Append(new Break() { Type = BreakValues.TextWrapping, Clear = BreakTextRestartLocationValues.Right });
                        runState.LastWasLineBreak = true;
                    }
                    else if (cw.Value.Value == 3)
                    {
                        EnsureRun().Append(new Break() { Type = BreakValues.TextWrapping, Clear = BreakTextRestartLocationValues.All });
                        runState.LastWasLineBreak = true;
                    }
                }
                return true;
